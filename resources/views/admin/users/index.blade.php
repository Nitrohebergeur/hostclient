    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div class="card">
            <div class="card-body py-4">
                ...
                <p class="text-sm text-{{ $color }}-600 dark:text-{{ $color }}-400 font-medium mt-1">{{ $label }}</p>
            </div>
        </div>
        @endforeach
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="table w-full">
                ...
                <tbody>
                    ...
                    ] as [$name, $email, $services, $total, $date, $status, $country])
                    <tr>
                        <td><input type="checkbox" class="w-4 h-4 rounded border-gray-300 text-primary-600"></td>
                        <td>
                            <div class="flex items-center gap-3">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($name) }}&background=0ea5e9&color=fff&size=36" class="w-9 h-9 rounded-full flex-shrink-0">
                                <div>
                        ...
                        <td class="text-center font-semibold text-gray-900 dark:text-white">{{ $services }}</td>
                        <td class="font-semibold text-gray-900 dark:text-white">{{ $total }}</td>
                        <td class="text-sm text-gray-600 dark:text-gray-400">{{ $date }}</td>
                        <td>
                            @if($status === 'active')    <span class="badge badge-success">Actif</span>
                            @elseif($status === 'suspended') <span class="badge badge-warning">Suspendu</span>
                            @else                        <span class="badge badge-danger">Banni</span>
                            @endif
                        </td>
                        <td>
