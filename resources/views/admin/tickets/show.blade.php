...
<div class="space-y-6">
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            ] as [$type, $name, $email, $time, $body, $private, $attachments])
            <div class="card {{ $private ? 'border-l-4 border-warning-400 bg-warning-50/30 dark:bg-warning-900/10' : ($type === 'staff' ? 'border-l-4 border-primary-500' : '') }}">
                <div class="card-body">
                    <div class="flex items-start justify-between gap-3 mb-4">
                        <div class="flex items-center gap-3">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($name) }}&background={{ $type === 'staff' ? ($private ? 'f59e0b' : '0ea5e9') : '6366f1' }}&color=fff&size=40" class="w-10 h-10 rounded-full flex-shrink-0">
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    <p class="font-semibold text-gray-900 dark:text-white text-sm">{{ $name }}</p>
                                    @if($type === 'staff' && !$private) <span class="badge badge-primary text-xs">Staff</span> @endif
                                    @if($private) <span class="badge badge-warning text-xs">🔒 Note interne</span> @endif
                                </div>
                                @if($email) <p class="text-xs text-gray-500 dark:text-gray-400">{{ $email }}</p> @endif
                            </div>
                        </div>
                    ...
                    <form class="space-y-4">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            ...
                            <div x-show="open" @click.away="open = false" x-transition class="absolute z-10 mt-1 w-72 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1">
                                ...
                                <button type="button" class="block w-full text-left px-4 py-2 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 leading-relaxed">{{ $quick }}</button>
                                @endforeach
                            </div>
                        </div>
