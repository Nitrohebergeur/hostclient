...
@foreach ($keys ?? $item->translatableKeys() as $key => $value)
<div id="translations-overlay-{{ $key }}" class="overflow-x-hidden overflow-y-auto hs-overlay hs-overlay-open:translate-x-0 translate-x-full fixed top-0 end-0 transition-all duration-300 transform h-full max-w-lg w-full w-full z-[80] bg-white border-s dark:bg-gray-800 dark:border-gray-700 hidden" tabindex="-1">
    ...
    <form method="POST" action="{{ route('admin.translations.index') }}" class="p-4">
        <input type="hidden" name="model" value="{{ $class ?? get_class($item) }}">
        <input type="hidden" name="model_id" value="{{ $id ?? $item->id }}">
        @csrf
        @foreach ($locales as $_key => $locale)
            ...
            @if ($value == 'text')
                @include('admin/shared/input', ['name' => "translations[". $_key . "][". $key ."]", 'value' => $item->trans($key, '', $_key)])
            @elseif ($value == 'editor')
                @include('admin/shared/editor', ['name' => "translations[". $_key . "][". $key ."]", 'value' => $item->trans($key, '', $_key)])
            @elseif ($value == 'textarea')
                @include('admin/shared/textarea', ['name' => "translations[". $_key . "][". $key ."]", 'value' => $item->trans($key, '', $_key)])
            @endif
        @endforeach
