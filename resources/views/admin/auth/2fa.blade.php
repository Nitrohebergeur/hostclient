...
<div class="p-4 sm:p-7">
    ...
    @if ($isTotpStep)
        <form method="POST" action="{{ route('admin.auth.2fa') }}" id="captcha-form" class="mt-5" novalidate>
            @csrf
    ...
    @else
        ...
        <div class="mt-4 flex flex-col items-center gap-2 text-sm">
            <form method="POST" action="{{ route('admin.auth.2fa.email') }}" class="contents">
                @csrf
                <button type="submit"
            ...
            @if ($requiresTotpBefore ?? false)
                <form method="POST" action="{{ route('admin.auth.2fa.reset') }}" class="contents">
                    @csrf
                    <button type="submit"
