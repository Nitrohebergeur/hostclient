...
<div class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-4 p-4 border border-gray-200 dark:border-gray-700 rounded-xl">
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-1">
                            <p class="font-medium text-gray-900 dark:text-white">{{ $key[0] }}</p>
                            <span class="badge {{ $loop->first ? 'badge-success' : 'badge-primary' }} text-xs">
                                {{ $loop->first ? 'Live' : 'Test' }}
                            </span>
                        </div>
                        <code class="text-sm text-gray-600 dark:text-gray-400 font-mono">{{ $key[1] }}</code>
                        <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">
                            Créée le {{ $key[2] }} · Expiration: {{ $key[3] }}
                        </p>
                    </div>
                    <div class="flex gap-2">
                        <button class="btn btn-secondary btn-sm" onclick="copyToClipboard('{{ $key[1] }}')">
    ...
    <div class="card">
        ...
        <div class="card-body space-y-3">
            ...
            <div class="flex items-center gap-4 p-3 rounded-xl {{ $session[4] ? 'bg-primary-50 dark:bg-primary-900/20' : 'bg-gray-50 dark:bg-gray-700/30' }}">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $session[0] }}</p>
                        @if($session[4])
                            <span class="badge badge-success text-xs">Cette session</span>
                        @endif
                    </div>
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ $session[1] }} · {{ $session[2] }} · {{ $session[3] }}</p>
                </div>
                @if(!$session[4])
