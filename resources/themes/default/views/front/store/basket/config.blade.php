...
<div class="{{ theme_metadata('layout_classes', 'max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto') }}">
    @php $activeCoupon = $row->basket->coupon_id ? $row->basket->coupon : null; @endphp
    <form id="basket-coupon-form" method="POST" action="{{ $activeCoupon ? route('front.store.basket.coupon.remove') : route('front.store.basket.coupon') }}"></form>
        ...
        <div class="grid grid-cols-3 gap-4">
            <div class="col-span-3 md:col-span-2">
                <div class="grid sm:grid-cols-3 gap-2 card" id="basket-billing-section">
                    ...
                    <label for="billing-{{ $pricing->recurring }}-{{ $pricing->currency }}" class="col-span-3 md:col-span-1 p-3 block w-full bg-white border border-gray-200 rounded-lg text-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400">
                        <span class="dark:text-gray-400 font-semibold">@if ($pricing->isFree()){{ __('global.free') }} @else {{ $pricing->getPriceByDisplayMode() }} {{ $pricing->getSymbol() }} @endif {{ $pricing->recurring()['translate'] }}
                            <p class="text-gray-500">{{ $pricing->pricingMessage() }} @if ($pricing->hasDiscountOnRecurring($product->getFirstPrice()))<span class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-medium bg-teal-100 text-teal-800 dark:bg-teal-800/30 dark:text-teal-500">-{{ $pricing->getDiscountOnRecurring($product->getFirstPrice()) }}%</span>@endif</p>
                        </span>
                ...
                <div class="card dark:text-gray-400">
                    ...
                    <input form="basket-coupon-form" type="hidden" name="_token" value="{{ csrf_token() }}">
                    @if ($activeCoupon)
                    <input form="basket-coupon-form" type="hidden" name="_method" value="DELETE">
                    @endif
                    <input form="basket-coupon-form" type="hidden" name="redirect_to" value="{{ request()->fullUrl() }}">
                    <div class="grid grid-cols-6 gap-2">
                        <div class="col-span-5">
                            @include('shared/input', [
                            'name' => 'coupon',
                            'attributes' => ['placeholder' => __('coupon.coupon_placeholder'), 'form' => 'basket-coupon-form'],
                            'value' => old('coupon', $activeCoupon?->code),
                            ])
                        </div>
                        <div>
                            @if ($activeCoupon)
                            <button form="basket-coupon-form" type="submit" class="mt-2 btn-danger py-3 px-4 w-full"><i class="bi bi-x-circle mr-3"></i> <span class="md:inline-block hidden">{{ __('coupon.remove_coupon') }}</span></button>
                            @else
                            <button form="basket-coupon-form" type="submit" class="mt-2 btn-primary py-3 px-4 w-full"><i class="bi bi-ticket-perforated mr-3"></i>
            ...
            <div class="col-span-3 md:col-span-1">
                <div class="card dark:text-gray-400">
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        <span id="coupon-code" class="text-primary">{{ $activeCoupon->code }}</span>
                    </div>
                    @endif
                    <div id="coupon-line" class="flex justify-between mb-2 {{ $activeCoupon ? '' : 'hidden' }}">
                        <span>{{ __('coupon.discount_price') }}</span>
                        <span id="coupon-discount" class="text-primary">0</span>
                    </div>

                    <div class="flex justify-between mb-2">
                        <span>{{ $activeCoupon ? __('coupon.subtotal_with_coupon') : __('store.subtotal') }}</span>
